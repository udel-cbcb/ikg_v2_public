...
class Simple(nn.Module):
    ...
    def __init__(self,
        ...
        self.rel_inv_embedding = nn.Embedding(self.num_relations,embedding_dim=self.embedding_dim)

        # init embeddings
        nn.init.uniform_(self.ent_embedding.weight.data,a=-1,b=1)
        nn.init.uniform_(self.rel_embedding.weight.data,a=-1,b=1)
        nn.init.uniform_(self.rel_inv_embedding.weight.data,a=-1,b=1)
    ...
    def calc_score(self,triples):
        ...
        r_inv_emb = self.rel_inv_embedding(r)
        t_emb = self.ent_embedding(t)

        score = (h_emb * r_emb) * t_emb
        score = torch.sum(score,dim=1)
        
        score_inv = (h_emb * r_inv_emb) * t_emb
        score_inv = torch.sum(score_inv,dim=1)        

        score_avg = (score+score_inv)/2
    ...
    def regularization(self, triples):
        ...
        regul = (torch.mean(h_emb ** 2) + torch.mean(t_emb ** 2) + torch.mean(r_emb ** 2) + torch.mean(r_inv_emb ** 2)) / 4
