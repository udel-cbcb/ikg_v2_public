...
from src.model.word2vec import Word2VecSkipGramTriple
import pandas as pd
...
class TripleWalkEmbedder(PyTorchTrial):
    def __init__(self, context: PyTorchTrialContext):
        ...
        # init model
        self.w2vmodel = Word2VecSkipGramTriple(num_nodes=len(self.entities_tensor),
    ...
    def _load_data(self):
        ...
        feature = self.data_config["feature_type"]
        fold_path = f"{data_dir}/processed/folds/fold_{fold_n}"
        fold_path_antd = f"{data_dir}/processed/annotated/{feature}/folds/fold_{fold_n}"

        logger.info(f"Reading triples from: {fold_path_antd}")
    ...
    def sample(self,nodes_batch):
        ...
        # generate contexts
        target_triples,pos_context,neg_context = rw.to_windows_triples(walks=walks,
        ...
        return target_triples,pos_context,neg_context
