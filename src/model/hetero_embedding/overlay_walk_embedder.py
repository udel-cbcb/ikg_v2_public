...
class OverlayWalkEmbedder(PyTorchTrial):
    ...
    def _load_data(self):
        ...
        relation_id_dict = train_entity_mapping_pd.set_index("entity").to_dict()['index']
        ...
        # get the phosph_id
        phosph_id = relation_id_dict["p"]
        logger.info(f"Phosph ID is : {phosph_id}")

        # drop phosph id from relations
        relation_id_dict.pop("p")

        # relation walk data
        relation_walk_data = {}

        # create walk data per relation
        for key, value in relation_id_dict:
            
            # subset the triples to include the relations of the given type
            target_relations = [phosph_id,value]
            train_triples_subset = train_triples_indexed_pd[train_triples_indexed_pd["relation"].isin(target_relations)]
            logger.info(f"Number of triples for {key} + p: {len(train_triples_subset)}")

            # drop relations key
            train_triples_subset = train_triples_subset.drop(columns=["relation"])
            train_subset_edges_tensor = torch.from_numpy(train_triples_subset.to_numpy())

            # build node edge index
            logger.info("Creating node edge index mapping")
            node_edge_idx, train_edges_tensor_sorted = rw_utils.build_node_edge_index(train_subset_edges_tensor,nodes_tensor)

            target_nodes_tensor = torch.unique(train_subset_edges_tensor.view(-1))
            target_nodes_tensor,_ = torch.sort(target_nodes_tensor)
            logger.info(f"Number of target nodes for {key} + p: {len(target_nodes_tensor)}")
        ...
        data = {
            ...
            "relation_walk_data": relation_walk_data
        }
