...
from src.model.trans_e import TransE
...
class TransEmbedder(PyTorchTrial):
    ...
    def _to_indexed_val_triples(self,val_edges,entity_id_mapping,relation_id_mapping):
        ...
        for index, val_edge in tqdm(val_edges.iterrows()):
            head = val_edge["head"]
            relation = "p"
            tail = val_edge["tail"]
    ...
    def _get_edge_embedding(self,model,triples):
        with torch.no_grad():
            ...
            r_emb= model.get_rel_embedding()[triples[:,1]]
