...
from src.model.word2vec import Word2VecCbow
import pandas as pd
...
class DeepWalkEmbedderCBow(PyTorchTrial):
    ...
    def train_batch(self, batch, epoch_idx: int, batch_idx: int):
        ...
        pos_nodes = batch[0]
        neg_nodes = batch[1]
        context_nodes = batch[2]

        # get the loss
        loss = self.model(pos_nodes,neg_nodes,context_nodes)
