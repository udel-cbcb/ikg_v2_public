...
def build():
    ...
    # load go embeddings
    go_embeddings_file = f"{data_dir}/processed/go/go_embeddings_{set}.csv"
    logger.info(f"Reading go embedding from: {go_embeddings_file}")
    go_embeddings = pd.read_csv(go_embeddings_file)
    go_embeddings = go_embeddings.set_index("go_term")

    # load protein to go mapping
    protein_to_go_mapping_file = f"{data_dir}/processed/go/proteins_to_go_mapping_{set}.csv"
    logger.info(f"Reading protein to go mapping from: {protein_to_go_mapping_file}")
    protein_to_go_mapping = pd.read_csv(protein_to_go_mapping_file)
    ...
    # save
    protein_to_go_embedding_file = f"{data_dir}/processed/go/protein_go_embeddings_{set}.csv"
    logger.info(f"Writing protein to go embedding to : {protein_to_go_embedding_file}")
    protein_go_embeddings_pd.to_csv(protein_to_go_embedding_file,index=False)
    logger.info("Done")
