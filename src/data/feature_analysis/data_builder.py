...
def _annotate_training_data(data_dir):
    ...
    for fold_n in range(folds):
        ...
        train_go_mf_graph = _annotate_with_go_terms(train_graph,proteins,"mf",data_dir)

        # serialize
        fold_dir = f"{data_dir}/processed/annotated/go_mf/folds/fold_{fold_n}"
        Path(fold_dir).mkdir(parents=True, exist_ok=True)
        kgb.serialize(train_go_mf_graph,f"{fold_dir}/train_triples.csv")

        # annotate with go bp terms
        train_go_bp_graph = _annotate_with_go_terms(train_graph,proteins,"bp",data_dir)

        # serialize
        fold_dir = f"{data_dir}/processed/annotated/go_bp/folds/fold_{fold_n}"
        Path(fold_dir).mkdir(parents=True, exist_ok=True)
        kgb.serialize(train_go_bp_graph,f"{fold_dir}/train_triples.csv")

        # annotate with pro terms
        train_pro_graph = _annotate_with_pro_terms(train_graph,proteins,pro_graph)

        # serialize
        fold_dir = f"{data_dir}/processed/annotated/pro/folds/fold_{fold_n}"
        Path(fold_dir).mkdir(parents=True, exist_ok=True)
        kgb.serialize(train_pro_graph,f"{fold_dir}/train_triples.csv")
        ...
        for biokg_group in biokg_groups.keys():
            group_relations = biokg_groups[biokg_group]

            logger.info(f"Annotating with : {biokg_group}")
            train_bio_kg_graph = _annotate_with_biokg_terms(train_graph,group_relations,biokg_edges)

            if biokg_group == "biokg_disease_ontology":
                # merge the ctd graph
                ...
                for ctd_edge in tqdm(ctd_edges):
                    ...
                    if head != "" and tail != "":
                        train_bio_kg_graph.add_edge(head,tail,edge_type="is_a")
                        train_bio_kg_graph.add_edge(tail,head,edge_type="has_a")

                    # check if has doid
                    head_doid = mesh_to_doid_dict.get(head)
                    if head_doid is not None and head_doid != "" and type(head_doid) != float:
                        train_bio_kg_graph.add_edge(head,head_doid,edge_type="is_a")
                        train_bio_kg_graph.add_edge(head_doid,head,edge_type="has_a")

                    
                    tail_doid = mesh_to_doid_dict.get(tail)
                    if tail_doid is not None and tail_doid != "" and type(tail_doid) != float:
                        train_bio_kg_graph.add_edge(tail,tail_doid,edge_type="is_a")
                        train_bio_kg_graph.add_edge(tail_doid,tail,edge_type="has_a")
            ...
            # serialize
            fold_dir = f"{data_dir}/processed/annotated/{biokg_group}/folds/fold_{fold_n}"
            Path(fold_dir).mkdir(parents=True, exist_ok=True)
            kgb.serialize(train_bio_kg_graph,f"{fold_dir}/train_triples.csv")
